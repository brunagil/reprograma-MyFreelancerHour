...
class App extends React.Component {
  constructor(props) { //criar o construtor que vai passar propriedades 
    super(props) 
    //State serve para tornar os componentes dinâmicos 
    this.state = {  //estabelecer as propriedades de state dos elementos
  ...
  setGanhoMes = (e) => {  //criar uma function que atualiza o valor da variável
    this.setState ({ //setState: *changes to the component state and tells React that this component and its children need to be re-rendered with the updated state.
  ...
  calculaValorHora = (e) => {
    ...
    this.setState({ // para atualizar o valor na tela! Chama o virtual DOM, o virtual DOM vê a diferença e ele altera a 
          ...
          <section className="secao secao-hora">
            <header className="secao__cabecalho">Calcule o seu valor/hora!</header>
              <article className="secao__conteudo">
                <form onSubmit={this.calculaValorHora} className="secao__formulario">
                  <div className="App">
                    <FieldSet 
                ...
                <footer className="secao__rodape"> 
                  <h3 className="secao__rodape__valor"><span>{this.state.valueHour}</span><small>/hora</small></h3>
